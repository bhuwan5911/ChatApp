...
export const getUserForSidebar = async (req, res)=>{
    try{
        ...
        const promises = filteredUsers.map(async (user)=>{
            const messages = await Message.find({senderId: user._id, receiveId:
                userId, seen:false})
        ...
        try {
            ...
            const messages = await  Message.find({
                $or: [
                    ...
                    {senderId: selectedUserIdId, receivedId: myId},

                ]
            }) 
            await Message.updateMany({ senderId: selectedUserId, receiverId, receivedId: myId},
                {seen : true});
